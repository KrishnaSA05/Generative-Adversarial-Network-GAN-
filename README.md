...
### Vanilla DCGAN — Epoch Progression

| Epoch 5 | Epoch 15 | Epoch 30 |
|:---:|:---:|:---:|
| Noisy scattered pixels | Distinct radar structures emerging | Sharp, diverse, realistic patterns |

**Observation:** The model generates recognisable radar signatures by Epoch 15.
By Epoch 30, images show clear structural diversity — cross patterns, horizontal
streaks, vertical columns, and blob signatures.

---

### Conditional DCGAN — Class-Specific Generation

**The key result** — each row below uses the **same random seed** but a different
class label. The Generator produces visually distinct signatures per class,
proving it has genuinely learned class-specific radar physics:

| Class | Visual Pattern | Physical Interpretation |
|---|---|---|
| 🚶 Pedestrian (0) | Wide, diffuse **horizontal blobs** | Small RCS, distributed reflection |
| 🛴 E-Scooter (1) | **Wider horizontal** + vertical detail | Larger combined rider + vehicle RCS |
| 🚗 Car (2) | Tall, sharp **vertical columns** | High RCS, strong specular reflection |
| 🌫️ Background (3) | Scattered **low-intensity clutter** | Random multipath reflections |
